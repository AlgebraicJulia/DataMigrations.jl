...
# Hmm, almost...But we got all the pairs of equal jobs as duplicated 
# and also got the one actual pair twice.
# Let's now write a migration to filter a `Conflicts`
# so that we don't get such extraneous information.
...
# This another limit migration; after running it we'll be left
# with only those `ConflictedPairs` whose `job1` is strictly
# less than their `job2`. The idea is that we want a diagram
# in `SchConflicts` including `ConflictedPairs` and `Bool`
# and two arrows, one checking `job1(x)<job2(x)` and one pointing
# to `true`. Then a consistent choice of elements for this diagram
# is simply a `ConflictedPair` such that `job1(x)<job2(x)`.
# In the current version of `DataMigrations`, we can't directly
# add `Bool` into a schema where it's not already present, so 
# we've somewhat inelegantly turned the desired booleans back into
# `Time`s. Anyway, let's see how we did!

B = migrate(Conflicts,A′,O)

# Voila, a correct list of conflicts! And of course we can now define
# something like

check_conflicts(A::Schedule) = migrate(Conflicts,migrate(Conflicts,A,N),O)

# Note that we don't currently directly compose the migrations `N` and `O`.
# This is mathematically possible but nontrivial to implement.
